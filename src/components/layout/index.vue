...
<template>
  <div class="flex layout-container">
    <SiderBar class="w-220px flex h-full flex-col p-4 glass-effect">
      <template #header>
        <div class="logo flex items-center justify-start">
          ...
          <div class="c-[var(--foreground-color)] font-700 line-height-6">{{ userStore.userInfo?.username || '未登录' }}
          </div>
        </div>
      </template>
    ...
    <div class="w-[calc(100%-220px)] p-4 border-[var(--border-color)] border-l-[2px] border-l-solid">
      <Header>
        <template #left>
          <div class="c-[var(--foreground-color)] flex items-center gap-2">
            ...
            <div class="flex flex-row items-center gap-1">
              <div class="font-900 font-size-5 select-none">{{ pageCode2TitleDict[currentPage] }}</div>
            </div>
          </div>
        </template>
        <template #center>
